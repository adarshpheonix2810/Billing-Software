...
class bill_window:
    ...
    def get_category(self, Event):
        ...
        self.combo2.set('')
        self.combo3.set('')
        find_subcat = "SELECT product_subcat FROM raw_inventory WHERE product_cat = %s"
    ...
    def get_subcat(self, Event):
        ...
        self.combo3.set('')
        find_product = "SELECT product_name FROM raw_inventory WHERE product_cat = %s and product_subcat = %s"
    ...
    def show_qty(self, Event):
        ...
        find_qty = "SELECT stock FROM raw_inventory WHERE product_name = %s"
    ...
    def add_to_cart(self):
        ...
        if strr.find('Total') == -1:
            ...
            if(product_name != ""):
                ...
                find_mrp = "SELECT mrp, stock FROM raw_inventory WHERE product_name = %s"
            ...
            if(cust_name.get() == ""):
                ...
                if strr.find('Total') == -1:
                    ...
                    for name, qty in self.cart.dictionary.items():
                        update_qty = "UPDATE raw_inventory SET stock = stock - %s WHERE product_name = %s"
    ...
    def search_bill(self):
        cur.execute("SELECT * FROM bill WHERE bill_no = %s", [cust_search_bill.get().rstrip()])
        results = cur.fetchall()
        if results:
            self.clear_bill()
            self.wel_bill()
